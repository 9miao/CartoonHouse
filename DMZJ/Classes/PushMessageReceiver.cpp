...
void PushMessageReceiver::onBind(std::string appid,std::string userId,std::string channelId,std::string requestId)
{
    ...
    HTTP_URL::S_HTTP_URL s_http_url = HTTP_URL::get_HTTP_URL(HTTP_URL::HttpUrlTypePushAdduser, userId);
    CommonHttpManager::getInstance()->send_post(s_http_url.url, s_http_url.key_value, this, CommonHttpJson_selector(PushMessageReceiver::onSumitPushUserIdFinish));
...
void PushMessageReceiver::bindUserOnLogin()
{
    ...
    if (user.uid.length()>0) {
        ...
        HTTP_URL::S_HTTP_URL s_http_url = HTTP_URL::get_HTTP_URL(HTTP_URL::HttpUrlTypePushBindinguser, user.uid, setting.userId);
        CommonHttpManager::getInstance()->send_post(s_http_url.url, s_http_url.key_value, this, CommonHttpJson_selector(PushMessageReceiver::onBindUserFinish));
    }
}
...
void PushMessageReceiver::unBindUserOnLogout()
{
    ...
    if (user.uid.length()>0) {
        ...
        HTTP_URL::S_HTTP_URL s_http_url = HTTP_URL::get_HTTP_URL(HTTP_URL::HttpUrlTypePushBindinguser, user.uid, setting.userId);
        CommonHttpManager::getInstance()->send_post(s_http_url.url, s_http_url.key_value, this, CommonHttpJson_selector(PushMessageReceiver::onUnBindUserFinish));
    }
}
